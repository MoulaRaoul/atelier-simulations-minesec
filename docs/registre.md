...
| G-08 | Une action se fait toujours de deux façons | Un objet de la scène peut être cliqué, mais tout ce qu'il déclenche reste accessible par un bouton du pupitre et au clavier : un objet en trois dimensions ne se sélectionne pas au clavier. L'objet cliquable à l'étape en cours luit en `--saisir`. | Simulation de l'eau, 16/09/2026 |
...
| Candidats au magasin : la vapeur et le clic sur un objet | Écrits dans `prototypes/eau-5e-cycle.html`, faute d'équivalent au magasin. La vapeur : des gouttelettes qui montent d'un récipient vers un nuage en s'effaçant. Le clic sur un objet : un appui relâché sans glisser, une visée par rayon sur l'objet actif, le curseur en main et la lueur `--saisir` (G-08). N'entrent au magasin qu'à la deuxième simulation qui en aura besoin. | Simulation de l'eau, 16/09/2026 |
